...
#    end
    ...
    title       = raw_data['data']['item']['title']
    description = 'Report from Rollbar'
    
    baseurl     = ENV['YOUTRACK_BASEURL']
    login       = ENV['YOUTRACK_LOGIN']
    password    = ENV['YOUTRACK_PASSWORD']
    project     = ENV['YOUTRACK_PROJECT']
    
    login = RestClient.post "#{baseurl}/rest/user/login",
      {
        :login    => login,
        :password => password
      }
      
    RestClient.put "#{baseurl}/rest/issue?project=#{project}&summary=#{title}&description=#{description}",
      {},
      {
